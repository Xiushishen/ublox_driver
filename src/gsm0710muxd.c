 ...
 *
 * Modified February 2018 by Bilal Ahmed <bilal.ahmed@u-blox.com>
 * Modified April 2020 by Muhammad Awais <muhammad.awais@u-blox.com>
 * Modified November 2020 by Bilal Khalid <bilal.khalid@u-blox.com>
 * Modified February 2022 by Uzair Afzal <uzair.afzal@u-blox.com>
 *
...
/***************************************************************************
  *
  * File modified by :  u-blox Lahore
  *
  ***************************************************************************/
...
//#include <features.h>
#include <paths.h>
#if defined (MUX_ANDROID) && ! defined (ANDROID_MM) && ! defined (ANDROID_NG) && ! defined (ANDROID_OO) && ! defined (ANDROID_PI)
#include <pathconf.h>
#endif
#include <signal.h>
#include <stdio.h>
...
#define MUX_ARGS_PROPERTY   "vendor.ubx.muxd.args"
#define MAX_MUX_ARGS        32
#else
...
/*Logging*/\
#ifndef MUX_ANDROID
#define LOG_TAG "NIXMUX"
#include <syslog.h>

#define LOG_VERBOSE     8   //Added for dumping MUX packets compatible to the python hexdump convertor tool
...
#define LOGMUX(lvl,f,...) do\
                          {\
                              ...
                              {\
                                  epoctime = time(NULL);\
                                  lt = localtime(&epoctime);\
                                  clock_gettime(CLOCK_REALTIME, &spec);\
                                  sprintf(timestr, "%02d-%02d-%04d %02d:%02d:%02d.%03d %c/%s:", lt->tm_mday, lt->tm_mon+1, lt->tm_year+1900, lt->tm_hour, lt->tm_min, lt->tm_sec, (int)(spec.tv_nsec / 1.0e6), loglevelStr(lvl),LOG_TAG);\
                                  if (logtofile)\
                                  {\
                                      if(lvl == LOG_VERBOSE)\
                                      {\
                                          fprintf(muxlogfile,"%s " f "\n", timestr, ##__VA_ARGS__);\
                                      }\
                                      else\
                                      {\
                                          fprintf(muxlogfile,"%s %d:%s(): " f "\n", timestr, __LINE__, __FUNCTION__, ##__VA_ARGS__);\
                                      }\
                                      fflush(muxlogfile);\
                                      if (lvl<LOG_INFO)\
                                      {\
                          ...
                          {\
                              ...
                              {\
                                  ...
                                  {\
                                      if(lvl == LOG_VERBOSE)\
                                      {\
                                          sprintf(timestr, "%02d-%02d-%04d %02d:%02d:%02d.%03d %c/%s:", lt->tm_mday, lt->tm_mon+1, lt->tm_year+1900, lt->tm_hour, lt->tm_min, lt->tm_sec, (int)(spec.tv_nsec / 1.0e6), loglevelStr(lvl-1),LOG_TAG);\
                                          fprintf(muxlogfile, "%s " f "\n", timestr, ##__VA_ARGS__);\
                                      }\
                                      else\
                                      {\
                                  ...
                                  {\
                                      if(lvl == LOG_VERBOSE)\
                                      {\
                                         LOG_PRI(android_log_lvl_convert[lvl - 1], LOG_TAG, f, ##__VA_ARGS__);\
                                      }\
                                      else\
                                      {\
                                         LOG_PRI(android_log_lvl_convert[lvl], LOG_TAG, "%d:%s(): " f, __LINE__, __FUNCTION__, ##__VA_ARGS__);\
...
#define GSM0710_MAX_TIMER    255
#define GSM0710_MAX_RETRANS  5
...
#define GSM0710_BUFFER_SIZE 1024*6
#define UBLOX_DEFAULT_CMUX_BAUDRATE 5 //115200 baud rate
...
void send_channel_close_frame(int channel, int sleep_time);
...
static char *ubxVersion = "$Ver: 04.02.00 $";
static int no_daemon = 0;
static int ctrl_exit = 1;
...
static struct timespec spec = {0, 0};
static struct tm *lt = NULL; // localtime

static int syslog_level = LOG_INFO;
static FILE *muxlogfile = NULL;
...
pthread_cond_t write_frame_signal = PTHREAD_COND_INITIALIZER;
...
// data length to be written on each channel
static int data_length_on_channel[GSM0710_MAX_CHANNELS] = {0};
// serial io
...
static int cmux_mode = 0; // u-blox module supports only basic cmux mode
//static int cmux_subset = 0;

static int cmux_port_speed = UBLOX_DEFAULT_CMUX_BAUDRATE; //115200 baud rate
static int flag_send_pwr_off_cmd = 0; // Send CPWROFF AT command during MUX close flag. Default value (0) is disabled, set to 1 to enable.
static int flag_hfc = 0; // Hardware flow control flag. By default disabled (0). Set to 1 to enable.

//TODO module specific parameters should be provided through commandline
#define MODULE_GENERIC 0
#define MODULE_SARA_R422 422
static int flag_module = MODULE_GENERIC;

// Maximum Frame Size (N1): 64/31
static int cmux_N1 = 1509;
// Supported for Linux only
#ifndef MUX_ANDROID
static int timer_ack = 253;
static int timer_ctrl = 254;
static int timer_wakeup = 0;
static int frame_retrans = 3;
#endif
#if 0
...
/*
 * char loglevelStr(int logLevel)
 * Purpose: Converts a log level value from INT to CHAR
 *
 * Input:   Log level in INT
 *
 * Return:  Returns log level in CHAR
 */
char loglevelStr(int logLevel)
{
    switch (logLevel)
    ...
    int length)
{
    ...
    unsigned int i, esc_i, adv_i = 0;

    for (i = 0; i < (unsigned int)length; ++i, ++adv_i)
    {
...
/*
 * int make_argv(int*, char**)
 * Purpose: Converts a single string of command line
 *          params to an array of strings
 * Input:   A string and a pointer to array of strings
 *
 * Return:  Returns the argc after string tokenization.
 *          -1 in case of overflows
 */
static int make_argv(char *args, char **argv, int size)
{
    int count = 1;
    char *tok;
    char *s = args;

    if ( size <= 0 || args == NULL || argv == NULL )
    {
        return -1;
    }
...
/*
* Purpose:   If two channels are in queue to write data to virtual port, wait for channel with less data to process first
* Input:     channel - channel number (0 = control)
*            length - the length of the data
* Return:    0
*/
static int write_scheduler_enter(int channel, int length)
{
    LOGMUX(LOG_DEBUG, "Enter channel %d", channel);
    int i = 0;
    data_length_on_channel[channel] = length;
    __sync_synchronize();
    // new mutex lock
    pthread_mutex_lock(&write_frame_lock);

    // Each channel threads want to write.
    // if another thread is waiting with less data to write
    // Be kind enough to let it go.
    for (i = 0; i < vir_ports; ++i)
    {
        if (data_length_on_channel[i] != 0 && data_length_on_channel[i] < length)
        {
            while (data_length_on_channel[i] != 0)
            {
                LOGMUX(LOG_ALERT, "Schedule channel %d (data=%d bytes) first due to less data instead of current channel %d (data=%d bytes)", i,
                       data_length_on_channel[i], channel, length);
                pthread_cond_wait(&write_frame_signal, &write_frame_lock);
                // Restart to process all channels.
                i = 0;
            }
        }
    }

    LOGMUX(LOG_DEBUG, "Leave channel %d", channel);
    return 0;
}

/*
* Purpose:   Signal and unlock mutex when channel data is completely processed
* Input:     channel - channel number (0 = control)
* Return:    0
*/
static int write_scheduler_exit(int channel)
{
    LOGMUX(LOG_DEBUG, "Enter channel %d", channel);
    data_length_on_channel[channel] = 0;
    pthread_cond_broadcast(&write_frame_signal);
    //unlock mutex
    ...
    unsigned char type)
{
    LOGMUX(LOG_DEBUG, "Enter channel %d", channel);
    /* new lock */
    write_scheduler_enter(channel, length);
    ...
    // T_HOST_DRV-148: According to official MUX spec, i.e. “ETSI TS 127 010 Multiplexer protocol” (3GPP TS 27.010), there are 2 types of C/R bits in MUX frames:
    // 1) C/R bit = 0/1 (at MUX frame level, i.e. in the “address” field)
    //      ------------------------|-----------------------------------|---------------|
    ...
    //      ------------------------|-----------------------------------|---------------|
    //      Initiator is the station that take the initiative to initialize the multiplexer (i.e. sends the SABM command at DLCI 0 ) and
    //      the responder is the station that accepts the initialization of the multiplexer (i.e. sends the UA response at DLCI 0).
    //      So in our case:
    //      a)  “Initiator” = DTE (host)    -> Terminal application
    //      b)  “Responder” = DCE (module)  -> u-blox modem
    // 2) C/R bit = 0/1 (at MSC message level, i.e. in the “control channel” field)
    ...
    //      Accordingly we have that the
    //      a)  C/R bit (at MSC message level, i.e. in the “control channel” field) has to be set to 1 for commands.
    //      b)  C/R bit (at MSC message level, i.e. in the “control channel” field) has to be set to 0 for responses.
    //
    //
    // Check if the input packet is MSC type with valid data length and its C/R bit (at MSC message level) is 0 (i.e. TE/initiator sending MSC response to modem/responder):
    // if so, also clear the C/R bit (at MUX frame level) in address field as per 3GPP MUX protocol.
    // NOTE: This check will not affect if TE/initiator sending MSC command to modem/responder with its C/R bit (at MSC message level) set to 1. In that case, C/R bit (at MUX frame level)
    // will also be set to 1 during 'prefix[5]' array initialization above.
    if (length > 0)
    {
        if ((unsigned char)input[0] == GSM0710_CONTROL_MSC)
        {
            prefix[1] &= ~GSM0710_CR;
            LOGMUX(LOG_WARNING, "Resetting C/R bit in MSC response packet to 0 on virtual channel %d", channel);
        }
    }
    ...
    if (!cmux_mode)
    {
        ...
        if (syslog_level == LOG_VERBOSE)
        {
            pthread_mutex_lock(&syslogdump_lock);/*new lock*/
            
            const int TEMP_BUF_SIZE = 64;
            char temp_buf[TEMP_BUF_SIZE];
            int print_count = 0, i = 0;
            memset(temp_buf, 0x00, TEMP_BUF_SIZE);
            
            LOGMUX(LOG_DEBUG, "Frame from Channel: %d", channel);
            LOGMUX(LOG_DEBUG, "len=[%d] : Raw Data To Serial Port: <<<<<<<<<<<<<<<", length);
            
            for (i = 0; i < prefix_length; i++)
            {
                print_count += snprintf(temp_buf + print_count, TEMP_BUF_SIZE - print_count, "%02X ", prefix[i]);
            }
            
            for (i = 0; i < length; i++)
            {
                print_count += snprintf(temp_buf + print_count, TEMP_BUF_SIZE - print_count, "%02X ", input[i]);

                if (print_count % 16 == 0)
                {
                    LOGMUX(LOG_VERBOSE, "\t%s", temp_buf);
                    print_count = 0;
                }
            }
            ...
            for(i = 0; i < 2; i++)
            {
                print_count += snprintf(temp_buf + print_count, TEMP_BUF_SIZE - print_count, "%02X ", postfix[i]);
...
int restart_pty_interface(Channel *channel)
{
    if (channel->fd < 0) // is this channel free?
    {
        char *pts;
#ifdef MUX_ANDROID
        char paramName[PROPERTY_KEY_MAX];
        char paramVal[PROPERTY_VALUE_MAX];
#endif
        ...
        // Get previous ptsname assigned to this channel->id
        sprintf(paramName, "vendor.ubx.muxd.ttydev.chan%d", channel->id);
        property_get(paramName, paramVal, "");

        if (strcmp(paramVal, channel->ptsname) != 0 )
        {
            // If ptsname assigned to this channel->id is changed, update also ptsname in RIL property
            LOGMUX(LOG_ALERT, "pts port assigned to channel %d property %s is changed from %s -> %s", channel->id, paramName, paramVal, channel->ptsname);
            property_set(paramName, channel->ptsname);
        }
        ...
        if (flag_hfc)
        {
            ...
            LOGMUX(LOG_ALERT, "HFC Enabled on serial device [%s]", channel->devicename);
        }
        else
        {
            options.c_cflag &= ~(CRTSCTS);
            LOGMUX(LOG_ALERT, "HFC Disabled on serial device [%s]", channel->devicename);
        }
        ...
        //create thread
        LOGMUX(LOG_ALERT, "Reopened %s, channel number: %d fd: %d ", channel->ptsname, channel->id, channel->fd);
        ...
        {
            LOGMUX(LOG_EMERG, "restart_pty_interface() - Out of memory\n");
            return 1;
        }
...
int pseudo_device_read(void *vargp)
{
    ...
    unsigned char buf[GSM0710_BUFFER_SIZE * 2];
    ...
    if (len >= 0)
    {
        ...
        if (channel->remaining > 0)
        {
            ...
            if (channel->tmp == NULL)
            {
                LOGMUX(LOG_EMERG, "pseudo_device_read() - Out of memory\n");
                return 1;
            }
...
static int c_alloc_channel(const char *origin, pthread_t *thread_id)
{
    ...
    if (serial.state == MUX_STATE_MUXING)
    {
        for (i = 1; i < GSM0710_MAX_CHANNELS; i++)
        {
            ...
            {
                LOGMUX(LOG_ALERT, "Found free channel %d fd %d on %s", i, channellist[i].fd, channellist[i].devicename);
...
#ifdef MUX_ANDROID
                // setup fixed tty device names for virtual pty channels created
                sprintf(device, "vendor.ubx.muxd.ttydev.chan%d", i);
                property_set(device, channellist[i].ptsname);
                LOGMUX(LOG_ALERT, "Set channel %d property: %s -> %s", i, device, channellist[i].ptsname);
#else // for linux OS
                sprintf(device, "/tmp/tty%d", i);

                if (symlink(pts, device) != 0)
                {
                    LOGMUX(LOG_ALERT, "Unable to create symlink");
                }
                ...
                {
                    options.c_cflag |= CRTSCTS;
                    LOGMUX(LOG_ALERT, "HFC Enabled on serial device [%s]", channellist[i].ptsname);
                }
                else
                {
                    options.c_cflag &= ~(CRTSCTS);
                    LOGMUX(LOG_ALERT, "HFC Disabled on serial device [%s]", channellist[i].ptsname);
                }
                ...
                //create thread
                LOGMUX(LOG_ALERT, "New channel properties: number: %d fd: %d device: %s", i, channellist[i].fd, channellist[i].devicename);
                Poll_Thread_Arg *poll_thread_arg = (Poll_Thread_Arg *)malloc(sizeof(Poll_Thread_Arg)); //iniitialize pointer to thread args

                if (poll_thread_arg == NULL)
                {
                    LOGMUX(LOG_EMERG, "c_alloc_channel() - Out of memory.\n");
                    return 1;
                }
                ...
                LOGMUX(LOG_ALERT, "Connecting %s to virtual channel %d for %s on %s",
...
{
    LOGMUX(LOG_DEBUG, "Enter");
    LOGMUX(LOG_INFO, "GSM0710 buffer (up-to-date): free %d, stored %d", gsm0710_buffer_free(buf), gsm0710_buffer_length(buf));
    int c = buf->endp - buf->writep;
    length = min((unsigned int)length, gsm0710_buffer_free(buf));
    ...
    LOGMUX(LOG_INFO, "GSM0710 buffer (up-to-date): written %d, free %d, stored %d", length, gsm0710_buffer_free(buf), gsm0710_buffer_length(buf));
    ...
    GSM0710_Buffer *buf)
{
    ...
    unsigned char *local_readp = NULL;
    ...
    if (!buf->flag_found)// no frame started
    {
        ...
        // u-blox Note: resolving a deadlock on buffer full as bufferready_signal was not being signalled in code.
        pthread_mutex_lock(&bufferready_lock);

        if (buf->input_sleeping == 1)
        {
            LOGMUX(LOG_DEBUG, "bufferready_signal");
    ...
    if (local_datacount >= (unsigned int)length_needed) /* enough data stored for 0710 frame header+footer? */
    {
        if ((frame = (GSM0710_Frame *)malloc(sizeof(GSM0710_Frame))) != NULL)
        {
            ...
            if (frame->channel > vir_ports ) /* Field Sanity check if channel ID actually exists */
            {
                ...
                free(frame); // frame->data is not allocated yet?
                buf->flag_found = 0;
                buf->dropped_count++;
        ...
        if (!(local_datacount >= (unsigned int)length_needed))
        {
            free(frame);
...
l_begin:
    ...
    while (gsm0710_buffer_length(buf) > 0)
    {
        ...
        if ((unsigned int)buf->adv_length >= sizeof(buf->adv_data)) /* frame data too much for buffer.. increase buffer size? */
        {
    ...
    int to)
{
    ...
    do
    {
        ...
        if (FD_ISSET(serial_device_fd, &rfds))
        {
            ...
            if (memstr((char *) buf, len, "SARA-R422"))
            {
                //TODO
                //xxx should be provided through commandline
                flag_module = MODULE_SARA_R422;
                LOGMUX(LOG_INFO, "Using SARA-R422 module");
                return 0;
            }
    ...
    if (frame->length > 0)
    {
        ...
        if ((type & GSM0710_CR) == GSM0710_CR)
        {
            ...
            if (supported)
            {
                ...
                // T_HOST_DRV-161: These changes are not according to the MUX protocol specifications so commented out
                #if 0
                switch ((type & ~GSM0710_CR))
                {
                    case GSM0710_CONTROL_MSC:
...
void signal_treatment(
    int param)
{
    LOGMUX(LOG_EMERG, "Interrupt signal [%d] is received\n", param);
    switch (param)
    {
        case SIGPIPE:
            exit(0);
        ...
        case SIGKILL:
        default:
            LOGMUX(LOG_EMERG, "Interrupt signal caught [%d]\n", param);
            exit(0);
            break;
    }
}
...
int thread_serial_device_read(void *vargp)
{
    ...
    LOGMUX(LOG_DEBUG, "Enter");
    {
        switch (serial->state)
        {
            ...
            {
                unsigned char buf[GSM0710_BUFFER_SIZE * 2] = {0};
                ...
                {
                    if ((len = read(serial->fd, buf, min((unsigned int)length, sizeof(buf)))) > 0)
                    {
                        if (syslog_level == LOG_VERBOSE)
                        {
                            pthread_mutex_lock(&syslogdump_lock);/*new lock*/
                            ...
                            LOGMUX(LOG_DEBUG, "len=[%d] : Raw Data From serial port: >>>>>>>>>>>>>>>", len);

                            for (i = 0; i < len; i++)
                            {
                                print_count += snprintf(temp_buf + print_count, TEMP_BUF_SIZE - print_count, "%02X ", buf[i]);
...
* Input:      com_speed - the baud_rates[] of serial port. Allowed range 0-8.
...
)
{
    ...
    {
        t.c_cflag |= CRTSCTS;
        LOGMUX(LOG_ALERT, "HFC Enabled on serial device [%s]", serial->devicename);
    }
    else
    {
        t.c_cflag &= ~(CRTSCTS);
        LOGMUX(LOG_ALERT, "HFC Disabled on serial device [%s]", serial->devicename);
    }
    ...
    speed_t speed = baud_bits[com_speed];
    ...
    LOGMUX(LOG_ALERT, "Switched Mux state to %d ", serial->state);
    return 0;
}
...
)
{
    ...
    //TODO
    SYSCHECK(chat(serial->fd, "AT+CGMM\r\n", 3));

    //xxx should be provided through commandline
    if (flag_module == MODULE_GENERIC)
    {
        if (flag_hfc)
        {
            SYSCHECK(chat(serial->fd, "AT&K3\r\n", 3));
            LOGMUX(LOG_ALERT, "HFC Enabled on module");
        }
        else
        {
            SYSCHECK(chat(serial->fd, "AT&K0\r\n", 3));
            LOGMUX(LOG_ALERT, "HFC Disabled on module");
        }
    }
...
#ifdef MUX_ANDROID
        // AT Command for TOBY-L, LARA-R, SARA-R4, LISA-U Series
        SYSCHECK(snprintf(gsm_command, sizeof(gsm_command), "AT+CMUX=%d,0,,%d,253,3,254\r\n",
                          cmux_mode, cmux_N1));
#else
        SYSCHECK(snprintf(gsm_command, sizeof(gsm_command), "AT+CMUX=%d,0,,%d,%d,%d,%d,%d\r\n"
                          , cmux_mode
                          //          , cmux_subset
                          //          , cmux_port_speed
                          , cmux_N1
    ...
    LOGMUX(LOG_NOTICE, "Starting mux mode");
...
static int close_devices()
{
    ...
    if (serial.fd >= 0)
    {
        send_channel_close_frame(0, 2); // u-blox change: sleep is introduced because MUX exited before sending close_channel_cmd for SARA modules
        serial.state = MUX_STATE_OFF;

        static const char *poff = "AT+CPWROFF\r\n";

        if (flag_send_pwr_off_cmd)
        {
            chat(serial.fd, "AT\r\n", 2);
            LOGMUX(LOG_DEBUG, "Sending AT+CPRWOFF now");
            chat(serial.fd, poff, 4);
        }
...
/*
* Purpose:    sending mux termination frames
* Input:      channel - channel number
              sleep_time - delay after sending termination frame
* Return:     0
*/
void send_channel_close_frame(int channel, int sleep_time)
{
    if (cmux_mode)
    {
        write_frame(channel, NULL, 0, GSM0710_CONTROL_CLD | GSM0710_CR);
    }
    else
    {
        write_frame(channel, close_channel_cmd, 2, GSM0710_TYPE_UIH);
    }

    sleep(sleep_time); // u-blox change: sleep is introduced because MUX exited before sending close_channel_cmd for SARA modules
}
...
static int watchdog(Serial *serial)
{
    ...
    switch (serial->state)
    {
        case MUX_STATE_OPENING:
            // u-blox change: Increased serial port baud rate for high data rate.
            LOGMUX(LOG_ALERT, "opening COM port at default baud [%d]", baud_rates[UBLOX_DEFAULT_CMUX_BAUDRATE]);

            if (open_serial_device(serial, UBLOX_DEFAULT_CMUX_BAUDRATE) != 0)
            {
                LOGMUX(LOG_WARNING, "Could not open serial device and start muxer");
                return 1;
            }

            LOGMUX(LOG_ALERT, "COM port opened successfully at default baud [%d]", baud_rates[UBLOX_DEFAULT_CMUX_BAUDRATE]);
            send_channel_close_frame(0, 2);

            if (cmux_port_speed != UBLOX_DEFAULT_CMUX_BAUDRATE)
            {
                ...
                if (chat(serial->fd, "AT\r\n", 3) < 0)
                {
                    LOGMUX(LOG_WARNING, "Serial not responding on Default Baud rate! trying Baud rate [%d]\r\n", baud_rates[cmux_port_speed]);
                    SYSCHECK(close(serial->fd));

                    if (open_serial_device(serial, cmux_port_speed) != 0)
                    {
                        LOGMUX(LOG_WARNING, "Could not open serial device and start muxer");
                        return 1;
                    }

                    LOGMUX(LOG_ALERT, "COM port opened successfully at baud [%d]", baud_rates[cmux_port_speed]);
                    send_channel_close_frame(0, 2);
                    chat(serial->fd, "AT\r\n", 3);
                ...
                // Sending AT command for module baud rate change to higher baud e.g. 921600.
                char cmd[100];
                SYSCHECK(snprintf(cmd, sizeof(cmd), "AT+IPR=%d\r\n", baud_rates[cmux_port_speed]));

                if (chat(serial->fd, cmd, 3) != 0)
                {
                    LOGMUX(LOG_ALERT, "Baud rate = %d not supported by the modem. Starting muxer at default baud rate (115200)\r\n", baud_rates[cmux_port_speed]);
                }
                else
                {
                    LOGMUX(LOG_ALERT, "Closing device to switch serial baud rate [%d]\r\n", baud_rates[cmux_port_speed]);
                    SYSCHECK(close(serial->fd));

                    LOGMUX(LOG_ALERT, "opening COM port at baud [%d]", baud_rates[cmux_port_speed]);
        ...
        case MUX_STATE_INITILIZING:
            ...
            if (poll_thread_arg == NULL)
            {
                LOGMUX(LOG_EMERG, "watchdog() - Out of memory\n");
                return 1;
            }
            ...
            LOGMUX(LOG_ALERT, "Multiplexing started..");
    ...
    char *_name)
{
    ...
    // process control
    fprintf(stderr, "\t-c: Send CPWROFF during MUX close. 0 (Disabled) - 1 (Enabled) [%s]\n", flag_send_pwr_off_cmd ? "yes" : "no");
    fprintf(stderr, "\t-F: Hardware Flow Control Setting. 0 (Disabled) - 1 (Enabled) [%d]\n", flag_hfc);
    fprintf(stderr, "\t-d: Fork, get a daemon [%s]\n", no_daemon ? "no" : "yes");
    fprintf(stderr, "\t-v: Set verbose logging level. 0 (Silent) - 7 (Debug), 8 (Wireshark Debug) [%d]\n", syslog_level);
    fprintf(stderr, "\t-C: Use this parameter to disable MUX exit on CTRL-C. CTRL-C exit is enabled by default. [%s]\n",
            ctrl_exit ? "ENABLED" : "DISABLED");
    // modem control
...
#ifndef MUX_ANDROID
    fprintf(stderr, "\t-N Maximum number of re-transmissions. Allowed range is 0-5. Default [%d]\n", frame_retrans);
    fprintf(stderr, "\t-A Acknowledgement timer in units of ten milliseconds. The allowed range is 1-255. Default [%d]\n", timer_ack);
    fprintf(stderr, "\t-T Response timer for the multiplexer control channel in units of ten milliseconds. The allowed range is 2-255. Default [%d]\n",
            timer_ctrl);
    fprintf(stderr, "\t-W Wake up response timer. The allowed range is 0-255. Default [%d]\n", timer_wakeup);
#endif
    fprintf(stderr, "\t-L <output log to file>: Output log to specified file. [%s]\n", logtofile ? "yes" : "no");
    //
...
void set_main_exit_signal(int signal)
{
    ...
    LOGMUX(LOG_ALERT, "main_exit_signal set [%d]", signal);
}
...
#ifdef MUX_ANDROID
//#define _PATH_MUXRUN  "/data/uril/"   //Compatible with older( <RIL v12.00 ) RIL version
#define _PATH_MUXRUN    "/data/vendor/uril/"
#else
#define _PATH_MUXRUN    "/tmp/"
#endif
...
create_pidfile(int pid)
{
    ...
    sprintf(pidfilename, "%s%s.pid", _PATH_MUXRUN, "muxd");
    ...
    {
        printf("Failed to create pid file %s: %m\n", pidfilename);
        pidfilename[0] = 0;
    }
}
...
int main(int argc, char *argv[])
{
...
#ifdef MUX_ANDROID
    char paramVal[PROPERTY_VALUE_MAX];
    char *newArgv[MAX_MUX_ARGS];
    property_get(MUX_ARGS_PROPERTY, paramVal, "");

    if (strcmp(paramVal, "") != 0)
    {
        argc = make_argv(paramVal, newArgv, MAX_MUX_ARGS);

        if (argc == -1)
        {
            LOGMUX(LOG_ALERT, "main() - Unable to process command line arguments through property [%s]. Exiting...", paramVal);
            return 1;
        }
...
#endif
    //for fault tolerance
    serial.devicename = "/dev/ttyUSB0";

    while ((opt = getopt(argc, muxArgv, "dCv:L:c:F:s:t:p:f:n:N:A:T:W:h?m:b:P:")) > 0)
    {
        switch (opt)
        {

            case 'c':   // case for power off command flag on mux_stop
                flag_send_pwr_off_cmd = atoi(optarg);
                break;

            case 'F':    // case for HFC enable/disbale
                flag_hfc = atoi(optarg);
                break;

            case 'C':  // case to disable Ctrl-C interupt
                ctrl_exit = 0;
                break;
            ...
            case 'v':
                ...
                if ((syslog_level > LOG_VERBOSE) || (syslog_level < 0))
                {
                    usage(argv[0]);
                    exit(0);
#ifdef MUX_ANDROID
            ...
            case 'L':
                ...
                if ((muxlogfile = fopen(optarg, "w+")) == NULL)
                {
                    fprintf(stderr, "Error: %s. Unable to open log file [%s]. Exiting...\n", strerror(errno), optarg);
                    usage(argv[0]);
                    exit(0);
                }
                else
            ...
            case 'N':
                frame_retrans = atoi(optarg);

                if ((frame_retrans > GSM0710_MAX_RETRANS) || (frame_retrans < 0))
                {
                    usage(argv[0]);
                    exit(0);
                }
            ...
            case 'A':
                timer_ack = atoi(optarg);

                if ((timer_ack > GSM0710_MAX_TIMER) || (timer_ack < 1))
                {
                    usage(argv[0]);
                    exit(0);
                }
            ...
            case 'T':
                timer_ctrl = atoi(optarg);

                if ((timer_ctrl > GSM0710_MAX_TIMER) || (timer_ctrl < 2))
                {
                    usage(argv[0]);
                    exit(0);
                }
            ...
            case 'W':
                timer_wakeup = atoi(optarg);

                if ((timer_wakeup > GSM0710_MAX_TIMER) || (timer_wakeup < 0))
                {
                    usage(argv[0]);
                    exit(0);
                }
    ...
    LOGMUX(LOG_EMERG, "%s %s starting", *argv, revision);
    //Initialize modem and virtual ports
    serial.state = MUX_STATE_OPENING;

    LOGMUX(LOG_ALERT, "Called with following options:");
    LOGMUX(LOG_ALERT, "u-blox Version: [%s]", ubxVersion);
    LOGMUX(LOG_ALERT, "\t-c: Send CPWROFF during MUX close [%s]", flag_send_pwr_off_cmd ? "yes" : "no");
    LOGMUX(LOG_ALERT, "\t-F: Hardware Flow Control Enabled [%s]", flag_hfc ? "yes" : "no");
    LOGMUX(LOG_ALERT, "\t-d: Fork, get a daemon [%s]", no_daemon ? "no" : "yes");
    LOGMUX(LOG_ALERT, "\t-v: Set verbose logging level. 0 (Silent) - 7 (Debug), 8 (Wireshark Debug) [%d]", syslog_level);
    LOGMUX(LOG_ALERT, "\t-C: Use this parameter to disable MUX exit on CTRL-C. CTRL-C exit is enabled by default. [%s]\n",
           ctrl_exit ? "ENABLED" : "DISABLED");
    LOGMUX(LOG_ALERT, "\t-s <serial port name>: Serial port device to connect to [%s]", serial.devicename);
    LOGMUX(LOG_ALERT, "\t-t <timeout>: reset modem after this number of seconds of silence [%d]", use_timeout);
    LOGMUX(LOG_ALERT, "\t-P <pin-code>: PIN code to unlock SIM [%d]", pin_code);
    LOGMUX(LOG_ALERT, "\t-p <number>: use ping and reset modem after this number of unanswered pings [%d]", use_ping);
    LOGMUX(LOG_ALERT, "\t-b <baudrate>: mode baudrate [%d]", baud_rates[cmux_port_speed]);
    LOGMUX(LOG_ALERT, "\t-m <modem>: Mode (basic, advanced) [%s]", cmux_mode ? "advanced" : "basic");
    LOGMUX(LOG_ALERT, "\t-f <framsize>: Frame size [%d]", cmux_N1);
    LOGMUX(LOG_ALERT, "\t-n <number of ports>: Number of virtual ports to create, must be in range 1-31 [%d]", vir_ports);
#ifndef MUX_ANDROID
    LOGMUX(LOG_ALERT, "\t-N Maximum number of re-transmissions. Allowed range is 0-5. Default [%d]\n", frame_retrans);
    LOGMUX(LOG_ALERT, "\t-A Acknowledgement timer in units of ten milliseconds. The allowed range is 1-255. Default [%d]\n", timer_ack);
    LOGMUX(LOG_ALERT, "\t-T Response timer for the multiplexer control channel in units of ten milliseconds. The allowed range is 2-255. Default [%d]\n",
           timer_ctrl);
    LOGMUX(LOG_ALERT, "\t-W Wake up response timer. The allowed range is 0-255. Default [%d]\n", timer_wakeup);
#endif
    LOGMUX(LOG_ALERT, "\t-L <output log to file>: Output log to provided file [%s]", logtofile ? "yes" : "no");

    create_pidfile(getpid());
    /** u-blox modification for ANDROID 5.0 **/
    /** This delay is introduced because previous versions of Android took relatively less time to boot up  **/
#ifdef ANDROID_LP
    LOGMUX(LOG_INFO, "Delay introduced");
    sleep(2);
#endif
    ...
    {
        LOGMUX(LOG_DEBUG, "GSM0710 buffer. Stored %d", gsm0710_buffer_length(serial.in_buf));
        LOGMUX(LOG_INFO, "Frames received/dropped: %d/%d", (int)serial.in_buf->received_count, (int)serial.in_buf->dropped_count);
        sleep(5);
    }
    ...
    LOGMUX(LOG_ALERT, "Received %ld frames and dropped %ld received frames during the mux-mode",
           serial.in_buf->received_count, serial.in_buf->dropped_count);
    LOGMUX(LOG_ALERT, "%s finished", argv[0]);
